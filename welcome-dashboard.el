...
(defvar welcome-dashboard-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "r") '(lambda () (interactive) (goto-line 9)))
...
(defun welcome-dashboard--insert-centered (margin text)
  ...
  (let* ((width (- (window-body-width) (* 2 margin) (length text) )))
  ...
  (with-current-buffer (get-buffer-create welcome-dashboard-buffer)
    ...
    (let* ((buffer-read-only)
      ...
      (let ((inhibit-read-only t))
        ...
        (when (display-graphic-p)
          (insert "\n\n")
          (let ((space (- (window-body-width) welcome-dashboard-path-max-length  (* 2 margin) (* 0.5 width) 2)
                ))
